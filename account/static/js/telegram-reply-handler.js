...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function cancelReply() {
        try {
            ...
            const activeButtons = document.querySelectorAll('.telegram-reply-btn.active');
            activeButtons.forEach(btn => btn.classList.remove('active'));
            
            // حذف کلاس selected-for-reply از همه پیام‌ها
            const selectedMessages = document.querySelectorAll('.telegram-message.selected-for-reply');
            selectedMessages.forEach(msg => msg.classList.remove('selected-for-reply'));
            
            // حذف همه پیش‌نمایش‌های ریپلای (برای اطمینان از عدم وجود چندین نمونه)
            const replyPreviews = document.querySelectorAll('.telegram-reply-preview');
    ...
    document.addEventListener('click', function(event) {
        ...
        if (event.target.closest('.telegram-conversation-item') || 
    ...
    function setupReplyEvents() {
        ...
        document.addEventListener('click', function(event) {
            ...
            if (event.target.closest('.telegram-replied-message')) {
                const repliedMessageId = event.target.closest('.telegram-replied-message').dataset.originalMessageId;
                if (repliedMessageId) {
    ...
    function selectMessageForReply(messageElement) {
        ...
        // حذف کلاس selected-for-reply از همه پیام‌ها
        document.querySelectorAll('.telegram-message.selected-for-reply').forEach(msg => {
        ...
        if (messageBody) {
            ...
            const attachmentItem = messageElement.querySelector('.telegram-attachment-item') || 
            ...
            if (fileBox) {
                ...
                // تشخیص نوع فایل
                if (fileBox.querySelector('audio') || fileBox.querySelector('.audio-player') || fileBox.classList.contains('audio-file')) {
                    fileType = 'audio';
                    messageText = messageText || 'فایل صوتی';
                    messageText = '🎵 ' + messageText;
                } else if (fileBox.querySelector('video') || fileBox.querySelector('.video-player') || fileBox.classList.contains('video-file')) {
                    fileType = 'video';
                    messageText = messageText || 'فایل ویدیویی';
                    messageText = '🎥 ' + messageText;
                } else if (fileBox.querySelector('img') || fileBox.classList.contains('image-file')) {
                    fileType = 'image';
                ...
                const fileName = attachmentItem.querySelector('.telegram-attachment-name') || 
                               attachmentItem.querySelector('.telegram-file-name') ||
                               attachmentItem.querySelector('.file-name');
                messageText = fileName ? fileName.textContent.trim() : '';
                
                // تشخیص نوع فایل
                if (attachmentItem.querySelector('img') || attachmentItem.classList.contains('image-attachment')) {
                    fileType = 'image';
                    messageText = messageText || 'تصویر';
                    messageText = '🖼️ ' + messageText;
                } else if (attachmentItem.querySelector('audio') || attachmentItem.classList.contains('audio-attachment')) {
                    fileType = 'audio';
                    messageText = messageText || 'فایل صوتی';
                    messageText = '🎵 ' + messageText;
                } else if (attachmentItem.querySelector('video') || attachmentItem.classList.contains('video-attachment')) {
                    fileType = 'video';
                    messageText = messageText || 'فایل ویدیویی';
                    messageText = '🎥 ' + messageText;
                } else {
        ...
        selectedMessageForReply = {
            id: messageId,
            text: messageText,
            previewText: messageText.length > 30 ? messageText.substring(0, 30) + '...' : messageText,
            fileType: fileType
        };
    ...
    function scrollToMessage(messageId) {
        const messageElement = document.querySelector(`.telegram-message[data-message-id="${messageId}"]`);
        if (messageElement) {
    ...
    function wrapExistingMessages() {
        // یافتن همه پیام‌ها
        const messages = document.querySelectorAll('.telegram-message:not(.wrapped)');
    ...
    function processRepliedMessages() {
        document.querySelectorAll('.telegram-message-body').forEach(messageBody => {
            ...
            if (messageText.startsWith('پاسخ به:')) {
                ...
                if (replyEndIndex !== -1) {
                    const repliedText = messageText.substring('پاسخ به:'.length, replyEndIndex).trim();
