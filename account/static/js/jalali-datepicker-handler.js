...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof window.loadJalaliDatepickerLibrary === 'function') {
        ...
        window._originalLoadJalaliDatepickerLibrary = window.loadJalaliDatepickerLibrary;
...
function setupDatepickerEvents() {
            ...
            if (gregorianDate && !isNaN(gregorianDate)) {
                ...
                if (testDate.getFullYear() > 2000) {
                    ...
                    if (Math.abs(testDate.getFullYear() - expectedGregorianYear) <= 1) {
                        ...
                        console.error(`سال شمسی ${jalaliYear} باید به حدود ${expectedGregorianYear} تبدیل شود، اما ${testDate.getFullYear()} شده است`);
                    }
                } else {
                ...
                // نمایش تاریخ در کنسول برای دیباگ
                // //('تاریخ و زمان انتخاب شده:', {
                //     jalali: jalaliDate,
                //     gregorian: gregorianDate,
                //     iso: window.scheduledTime,
                //     year: testDate.getFullYear(),
                //     month: testDate.getMonth() + 1,
                //     day: testDate.getDate(),
                //     expectedYear: expectedGregorianYear
                // });
    ...
    if (scheduleTimeDisplay) {
        ...
        if (scheduleTimeDisplay.textContent === 'انتخاب نشده' || !scheduleTimeDisplay.textContent.trim()) {
            scheduleTimeDisplay.classList.add('not-selected');
            scheduleTimeDisplay.classList.remove('selected');
        } else {
            scheduleTimeDisplay.classList.add('selected');
...
function jalaliToGregorian(jDate) {
    ...
    try {
        ...
        let second = timeParts.length > 2 ? parseInt(toEnglishDigits(timeParts[2])) : 0;
...
function getJalaliDate(date) {
    try {
        ...
        function gregorianToJalali(gy, gm, gd) {
            ...
            gy2 = (gm > 2) ? (gy + 1) : gy;
            d = g_d_m[gm - 1] + gd + parseInt((gy2 + 3) / 4) - parseInt((gy2 + 99) / 100) + parseInt((gy2 + 399) / 400) - 80;
        ...
        // ساخت رشته نهایی
        return `${jalali[0]}/${pad(jalali[1])}/${pad(jalali[2])} ${pad(hours)}:${pad(minutes)}`;
    } catch (error) {
