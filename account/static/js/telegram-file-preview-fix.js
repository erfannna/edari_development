...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.updateUIAfterSend = function(response) {
        ...
        try {
            ...
            const allIcons = document.querySelectorAll('.telegram-input-icon, .telegram-option-btn');
            ...
            if (inputOptions) {
                ...
                const isInActiveChat = chatContainer && chatContainer.classList.contains('telegram-container-active');
        ...
        try {
            ...
            if (recordVoiceBtn) {
                ...
                if (recordVoiceBtn.parentNode) {
                    recordVoiceBtn.parentNode.replaceChild(newRecordVoiceBtn, recordVoiceBtn);
                }
            ...
            if (telegramInput) {
                ...
                if (telegramInput.parentNode) {
                    telegramInput.parentNode.replaceChild(newTelegramInput, telegramInput);
                }
    ...
    window.resetFileUploadSystem = function() {
        ...
        try {
            ...
            if (oldFileInput) {
                ...
                const fileInputAccept = oldFileInput.accept || 'image/*,audio/*,video/*,.pdf,.docx,.xlsx';
            ...
            if (oldAttachFileBtn) {
                ...
                if (oldAttachFileBtn.parentNode) {
                    oldAttachFileBtn.parentNode.replaceChild(newAttachFileBtn, oldAttachFileBtn);
    ...
    function updateInputOptionsVisibility() {
        ...
        if (chatContainer && inputOptions) {
            const isInActiveChat = chatContainer.classList.contains('telegram-container-active');
    ...
    window.createSendFileButton = function() {
        ...
        if (inputContainer) {
            ...
            sendFileBtn.addEventListener('click', function(event) {
                ...
                // بررسی وضعیت زمان‌بندی و پنل زمان‌بندی
                const schedulePanel = document.getElementById('schedulePanel');
                const scheduledTimeInput = document.getElementById('scheduledTimeInput');
                
                // بررسی اگر تاریخ انتخاب شده است (وجود متغیر scheduledTime)
                if (window.scheduledTime && scheduledTimeInput && scheduledTimeInput.value.trim() !== '') {
                    //('تاریخ قبلاً انتخاب شده است، ارسال فایل...');
                    
                    // ارسال فایل
                    sendFile();
                    
                    // بازنشانی پس از ارسال - بدون تاخیر
                        resetAfterSend(true);
                    
                    return; // از ادامه اجرا جلوگیری می‌کنیم
                }
                
                // در هر بار کلیک، وضعیت زمان‌بندی را بازنشانی می‌کنیم
                window.isScheduleActive = false;
                ...
                if (schedulePanel) {
                    ...
                    // اگر زمان‌بندی فعال نیست، ابتدا پنل زمان‌بندی را نمایش می‌دهیم
                if (!window.isScheduleActive) {
                    //('نمایش پنل زمان‌بندی برای فایل');
                    
                    // بررسی وجود تابع showSchedulePanel
                    if (typeof window.showSchedulePanel === 'function') {
                        // فراخوانی تابع showSchedulePanel با پارامتر true برای نمایش پنل و 'attachment' برای نوع محتوا
                        try {
                        ...
                        window.isScheduleActive = true;
                            ...
                            setTimeout(() => {
                                ...
                                if (schedulePanel) {
                                    ...
                                    //('پنل زمان‌بندی با موفقیت نمایش داده شد');
                                    
                                    // اطمینان از اینکه فیلد انتخاب تاریخ فعال است
                                    const scheduledTimeInput = document.getElementById('scheduledTimeInput');
                                    if (scheduledTimeInput) {
                                        try {
                                            scheduledTimeInput.focus();
                                            scheduledTimeInput.click();
                                            //('فیلد انتخاب تاریخ فعال شد');
                                            
                                            // اگر فیلد دارای رویداد کلیک است، آن را فعال می‌کنیم
                                            if (typeof scheduledTimeInput.showPicker === 'function') {
                                                scheduledTimeInput.showPicker();
                                                //('انتخابگر تاریخ به صورت برنامه‌ای باز شد');
                                            } else {
                                                ...
                                                //('رویداد کلیک روی فیلد تاریخ شبیه‌سازی شد');
                                            }
                                        } catch (e) {
                                    ...
                                    console.error('پنل زمان‌بندی یافت نشد!');
                                    // اگر پنل زمان‌بندی یافت نشد، مستقیماً فایل را ارسال می‌کنیم
                    sendFile();
                                    resetAfterSend(true);
                                }
                            }, 100);
                            ...
                            // در صورت خطا، فایل را مستقیماً ارسال می‌کنیم
                            sendFile();
                            resetAfterSend(true);
                        }
                    } else {
                        console.error('تابع showSchedulePanel یافت نشد! ارسال مستقیم فایل...');
                        // اگر تابع showSchedulePanel وجود ندارد، مستقیماً فایل را ارسال می‌کنیم
                        sendFile();
                        resetAfterSend(true);
                    }
                } else {
                    // اگر زمان‌بندی فعال است، فایل را ارسال می‌کنیم
                    //('زمان‌بندی فعال است، ارسال فایل...');
                    sendFile();
                    resetAfterSend(true);
                }
            });
            ...
            sendFileBtn.addEventListener('mouseover', function() {
                this.style.backgroundColor = '#1A7AAB'; // رنگ آبی تیره‌تر
                this.style.transform = 'scale(1.05)';
            });
            
            sendFileBtn.addEventListener('mouseout', function() {
                this.style.backgroundColor = '#229ED9'; // برگشت به رنگ اصلی
    ...
    window.sendFile = function() {
        ...
        if (!chatBody) {
            ...
            if (activeChatElement) {
                ...
                const chatBodyInActive = activeChatElement.querySelector('.telegram-chat-body');
                if (chatBodyInActive) {
            ...
            window.sendMessage = function() {
                ...
                function updateUploadProgressUI(percent) {
                    ...
                    if (percent >= 100) {
                            ...
                            spinner.innerHTML = '<i class="fa-solid fa-check" style="color: #8effb5; font-size: 16px;"></i>';
                    ...
                    const allTimeElements = document.querySelectorAll('.telegram-message-time');
                    allTimeElements.forEach((el) => {
                        if (el.textContent.includes('در حال') || el.textContent.includes('%')) {
                            if (percent < 100) {
                ...
                xhr.addEventListener('load', function() {
                    ...
                    updateUploadProgressUI(100);
                        ...
                        const messageTime = progressElement.querySelector('.telegram-message-time');
                        if (messageTime) {
                    ...
                    if (xhr.status >= 200 && xhr.status < 300) {
                        try {
                            ...
                            if (progressElement && progressElement.parentNode) {
                                const uploadTitle = progressElement.querySelector('.telegram-message-subject');
                                if (uploadTitle) {
                ...
                xhr.addEventListener('error', function() {
                    ...
                    if (progressElement && progressElement.parentNode) {
                        const progressText = progressElement.querySelector('.upload-progress-percent');
                        if (progressText) {
    ...
    function updateUploadProgress(percent) {
        ...
        const allProgressTexts = document.querySelectorAll('.upload-progress-percent');
    ...
    document.addEventListener('schedule-selected', function(event) {
        ...
        const filePreview = document.getElementById('filePreview');
        if (filePreview && filePreview.style.display === 'flex' && window.selectedFile) {
        ...
        mutations.forEach(function(mutation) {
            if (mutation.type === 'childList') {
                ...
                mutation.addedNodes.forEach(function(node) {
                    if (node.nodeType === 1) { // المنت DOM
                        // بررسی اگر المنت اضافه شده یکی از عناصر مورد نظر ماست
                        if (node.id === 'attachFileBtn' || node.querySelector('#attachFileBtn')) {
...
function createUploadProgressMessage() {
    ...
    if (fileType.startsWith('image/')) {
        ...
        fileIcon = '<i class="fa-solid fa-file-excel" style="font-size: 22px;"></i>';
    } else {
...
window.createAttachmentPreview = function(attachment, messageElement) {
    ...
    if (!attachment || !messageElement) {
        console.error('پارامترهای ورودی برای تابع createAttachmentPreview ناقص است');
        return;
    }
    ...
    if (fileType.includes('image')) {
        fileIcon = '<i class="fa-regular fa-image" style="margin-left: 8px; font-size: 22px;"></i>';
    } else if (fileType.includes('audio')) {
        fileIcon = '<i class="fa-solid fa-music" style="margin-left: 8px; font-size: 22px;"></i>';
    } else if (fileType.includes('video')) {
        fileIcon = '<i class="fa-solid fa-video" style="margin-left: 8px; font-size: 22px;"></i>';
    } else if (fileType.includes('pdf')) {
        fileIcon = '<i class="fa-solid fa-file-pdf" style="margin-left: 8px; font-size: 22px;"></i>';
    } else if (fileType.includes('word') || fileType.includes('document')) {
        fileIcon = '<i class="fa-solid fa-file-word" style="margin-left: 8px; font-size: 22px;"></i>';
    } else if (fileType.includes('excel') || fileType.includes('sheet')) {
        fileIcon = '<i class="fa-solid fa-file-excel" style="margin-left: 8px; font-size: 22px;"></i>';
        } else {
        fileIcon = '<i class="fa-solid fa-file" style="margin-left: 8px; font-size: 22px;"></i>';
    }
