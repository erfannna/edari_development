...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const telegramChatContainer = document.getElementById('telegramChatContainer');
    ...
    function clearModalOverlay() {
        ...
        if (filePreview && window.selectedFile) {
            ...
            const sendButton = telegramChatContainer.querySelector('#telegramSendBtn');
            if (sendButton) {
                sendButton.style.display = 'none';
            }
            
            const allSendButtons = telegramChatContainer.querySelectorAll('.telegram-send-btn');
    ...
    window.resetChatData = function() {
        ...
        try {
            ...
            const chatInputs = telegramChatContainer.querySelectorAll('.telegram-chat-input, .telegram-message-input, .telegram-input, textarea[data-role="telegram-input"], .chat-input');
            ...
            const editableContents = telegramChatContainer.querySelectorAll('[contenteditable="true"]');
            ...
            const fileInputs = telegramChatContainer.querySelectorAll('input[type="file"]');
            ...
            const scheduledTimeInput = telegramChatContainer.querySelector('#scheduledTimeInput');
            ...
            const scheduleTimeDisplay = telegramChatContainer.querySelector('#scheduleTimeDisplay');
            ...
            if (typeof jalaliDatepicker !== 'undefined' && isTelegramActive) {
                ...
                const jdpContainer = telegramChatContainer.querySelector('.jdp-container');
    ...
    function improveSchedulePanelStyle() {
        ...
        if (schedulePanel) {
            ...
            const removeBtn = schedulePanel.querySelector('.telegram-remove-schedule');
            if (removeBtn) {
    ...
    window.showSchedulePanel = function(setDefaultDate = false, contentType = 'text') {
        // بررسی اینکه آیا کامپوننت تلگرام فعال است
        const isTelegramActive = telegramChatContainer.classList.contains('open');
        if (!isTelegramActive) {
        ...
        if (typeof jalaliDatepicker !== 'undefined' && isTelegramActive) {
            ...
            if (typeof window.loadJalaliDatepickerLibrary === 'function' && isTelegramActive) {
    ...
    const telegramCloseBtn = telegramChatContainer.querySelector('#telegramClose');
    if (telegramCloseBtn) {
    ...
    if (telegramChatContainer.classList.contains('open')) {
        ...
        const chatContainer = telegramChatContainer.querySelector('.telegram-chat-body, .telegram-messages-container, #telegramChatBody');
        if (chatContainer) {
                        ...
                        for (const node of mutation.addedNodes) {
                            if (node.classList && 
                               (node.classList.contains('telegram-file-preview') || 
                                (node.querySelector && node.querySelector('.telegram-file-preview')) ||
    ...
    document.addEventListener('jdp:change', function(event) {
        ...
        if (telegramChatContainer.classList.contains('open') && 
            event.target && telegramChatContainer.contains(event.target)) {
            // پاکسازی overlay
            clearModalOverlay();
